...
function StepIndicator({ status, icon }: { status: string; icon: string }) {
    ...
    if (status === 'completed') {
        return (
            <div className={`${base} bg-emerald-500/15 ring-1 ring-emerald-500/30`}>
                <span className="material-symbols-outlined text-emerald-400 text-xl">check</span>
            </div>
        );
    }
    ...
    if (status === 'processing') {
        return (
            <div className={`${base} bg-[#5555f6]/15 ring-1 ring-[#5555f6]/40 animate-pulse`}>
                <span className="material-symbols-outlined text-[#5555f6] text-xl">{icon}</span>
            </div>
        );
    }
...
function Connector({ done }: { done: boolean }) {
    return (
        <div className="flex justify-center py-1">
            <div className={`w-0.5 h-5 rounded-full transition-colors duration-500 ${done ? 'bg-emerald-500/40' : 'bg-slate-700/40'}`} />
        </div>
    );
}

export default function VideoProcessingPreview({ videoId }: { videoId: number | null }) {
    const { steps, STEPS, STEP_LABELS, failed, completed, videoUrl } = useVideoProcessing(videoId);

    if (!videoId) {
        return (
            <div className="text-center space-y-4">
                <div className="w-16 h-16 rounded-2xl bg-slate-800/60 flex items-center justify-center mx-auto ring-1 ring-slate-700/40">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-600 mt-1">Generate a video to get started</p>
                </div>
            </div>
        );
    }
                    ...
                    return (
                        <div key={step}>
                            <div className="flex items-center gap-3">
                                <StepIndicator status={status} icon={STEP_ICONS[step]} />
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-medium transition-colors duration-300 ${
                                        status === 'processing' ? 'text-slate-100' :
                                    ...
                                    <p className="text-[11px] text-slate-600">
                                        {status === 'processing' ? 'In progress...' :
                                         ...
                                         status === 'failed' ? 'Error occurred' : 'Waiting'}
                                    </p>
                                </div>
                            </div>
                            {i < STEPS.length - 1 && <Connector done={steps[STEPS[i + 1]] !== 'waiting'} />}
                        </div>
                    );
                })}
            </div>
